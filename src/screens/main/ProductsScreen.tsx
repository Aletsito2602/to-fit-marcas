...
type ProductsNavigationProp = StackNavigationProp<ProductsStackParamList, 'ProductsList'>;

// Modal de Selección de Tipo de Producto
interface ProductTypeModalProps {
  visible: boolean;
  onClose: () => void;
  onSelectType: (type: 'individual' | 'collection') => void;
}

const ProductTypeModal: React.FC<ProductTypeModalProps> = ({ visible, onClose, onSelectType }) => {
  const handleSelectType = (type: 'individual' | 'collection') => {
    onSelectType(type);
    onClose();
  };
      ...
      <View style={modalStyles.overlay}>
        <View style={modalStyles.container}>
          <View style={modalStyles.header}>
            <Text style={modalStyles.title}>Crear Producto</Text>
          ...
          <View style={modalStyles.optionsContainer}>
            ...
            >
              <View style={modalStyles.optionIcon}>
                <Ionicons name="shirt-outline" size={32} color="#0FFF95" />
              </View>
              <Text style={modalStyles.optionTitle}>Producto Individual</Text>
              <Text style={modalStyles.optionDescription}>
                Crear un producto único con sus propias características
              </Text>
            </TouchableOpacity>
            ...
            >
              <View style={modalStyles.optionIcon}>
                <Ionicons name="library-outline" size={32} color="#9747FF" />
              </View>
              <Text style={modalStyles.optionTitle}>Colección</Text>
              <Text style={modalStyles.optionDescription}>
                Crear una colección con múltiples productos relacionados
              </Text>
            </TouchableOpacity>
          </View>
        </View>
...
const ProductsScreen: React.FC = () => {
  ...
  const handleSelectProductType = (type: 'individual' | 'collection') => {
    if (type === 'individual') {
      navigation.navigate('AddEditProduct', {});
    } else {
      // TODO: Navegar a pantalla de crear colección
      Alert.alert('Próximamente', 'La creación de colecciones estará disponible pronto');
    }
  };
